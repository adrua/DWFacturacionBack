// CntBancos - APIRest - OData Testing
using System;
...
{
    public class CntBancosTest
    : IClassFixture<CustomWebApplicationFactory<Startup>>
    {
        #region Tests
        [Theory]
        [InlineData("/odata/v1/CntBancos(BancoId=1001)")]
        ...
        [Theory]
        [InlineData("/CntBancos(BancoId=1001)")]
        ...
        [Theory]
        [InlineData("/CntBancos(BancoId=-1)")]
        ...
        [Theory]
        [InlineData("/CntBancos(BancoId=1003)")]
        ...
        private static CntBancos rowBase = new CntBancos() {
            BancoId = 1001,
            BancoNombre = $@"Banco de Bogota",
            BancoLongitud = 20,
        };

        private static CntBancos rowAdd = new CntBancos() {
            BancoId = 1002,
            BancoNombre = $@"Canco de Bogota",
            BancoLongitud = 21,
        };

        private static CntBancos rowDelete = new CntBancos() {
            BancoId = 1003,
            BancoNombre = $@"Danco de Bogota",
            BancoLongitud = 22,
        };

        public CntBancosTest(CustomWebApplicationFactory<Startup> factory)
        {
            ...
            if (factory.Db == null)
            {
                ...
                {
                    if (db.CntBancos.Count() == 0)
                    {
                        db.CntBancos.Add(rowBase);
                        db.CntBancos.Add(rowDelete);
                        db.SaveChanges();
                    }
